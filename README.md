...
**Current development: terminal/editor continuity, Herdr bindings, artifact versions and CLI/TUI progress review, extending 0.7.0 with database schema 17.** Resume an explicit task with its accepted direction, saved progress, blockers and file drift. Save progress through editable checkpoint drafts that preserve their original task, revision and retry identity.
...
For extension installation, use [Browser setup](docs/BROWSER-SETUP.md). The extension and daemon run together; the browser launches the native helper. Load `extension/` unpacked, or extract `bin/heimdall-extension-0.4.0.zip`. Native-host registration is a separate local installation step.
...
The current binary upgrades database markers 1–16 to **17**, publishing a consistent `backups/pre-schema-17-*.db` before migration. Older binaries refuse marker 17. Database backups preserve recorded state and receipts; external working files require separate preservation. Follow [backup, upgrade and restore](docs/CONTINUITY-SETUP.md#backup-upgrade-and-restore) for fresh-directory recovery or rollback.
