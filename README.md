...
**Current development: terminal/editor continuity, Herdr bindings, artifact versions and CLI/TUI progress review, extending 0.7.0 with database schema 16.** Resume an explicit task with its accepted direction, saved progress, blockers and file drift. Save progress through editable checkpoint drafts that preserve their original task, revision and retry identity.
...
| Workspace/session identity | Initial W01/T02 delivered: explicit manifests, generic declarations and verified local Herdr bindings with refresh/metadata. Schema-9 migration/replay tested. W02 adds read-only Hyprland observation and explicit viewport binding. W03 adds durable manual points and opt-in autosnapshots. W04 adds scoped List/Diff/preview and stale-plan validation. Recovery actions remain open. |
...
For extension installation, use [Browser setup](docs/BROWSER-SETUP.md). The extension and daemon run together; the browser launches the native helper. Load `extension/` unpacked, or extract `bin/heimdall-extension-0.3.0.zip`. Native-host registration is a separate local installation step.
...
| `action context\|queue\|show\|list\|history\|cancel TASK` | Shared task-bound intent, attempt history and explicit cancellation; API execution and verification remain separate |
...
The current binary upgrades database markers 1–15 to **16**, publishing a consistent `backups/pre-schema-16-*.db` before migration. Older binaries refuse marker 16. Database backups preserve recorded state and receipts; external working files require separate preservation. Follow [backup, upgrade and restore](docs/CONTINUITY-SETUP.md#backup-upgrade-and-restore) for fresh-directory recovery or rollback.
