...
func Normalize(r *model.Resource) error {
...
// PrepareResource uses the same canonicalization and bounded observation as CLI binding.
func PrepareResource(ctx context.Context, r *model.Resource) error {
	if err := Normalize(r); err != nil {
		return err
	}
	var err error
	r.Initial, err = Observe(ctx, *r)
	return err
}
