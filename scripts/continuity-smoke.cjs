...
(async () => {
  const {root, exe, dir} = require('./smoke-paths.cjs').smokePaths('continuity');
  ...
  try {
    ...
    if (legacy) assert.equal(fs.readdirSync(join(data, 'backups')).filter(n => n.startsWith('pre-schema-18-')).length, 1);
    ...
    if (legacy) {
      ...
      assert.notEqual(refused.status, 0); assert.match(refused.stderr, /unsupported database schema version 18/);
    }
    const source = data;
    ...
    if (legacy) {
      ...
      const pre = fs.readdirSync(join(source, 'backups')).find(n => n.startsWith('pre-schema-18-'));
