...
16 introduced this history; current schema 21 preserves it. Older browser operations remain unchanged; `browser status` exposes them in
...
Use extension 0.6.0 with this daemon. C13 adds [challenged browser verification](BROWSER-VERIFICATION.md) and `action reconcile`; the C12 historical contract below remains readable. The public development key and extension ID
...
C13 now verifies independent browser postconditions and actual isolated Linux native registration. Browser/compositor association is delivered; see [pairing](BROWSER-PAIRING.md).
...
Current supported upgrades publish `backups/pre-schema-21-*.db` before
migration to schema 21. Older binaries refuse marker 21. Roll back only into a fresh directory from the
