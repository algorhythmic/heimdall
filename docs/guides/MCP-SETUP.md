# MCP and scoped progress writes
...
Database schema 5 introduced version-2 write grants and client checkpoints; schema 6 added evaluator/evidence records without broadening MCP authority. Old read grants, CLI checkpoints and contract versions remain replayable. Current schema 21 upgrades supported older databases after a consistent `pre-schema-21-*.db` backup. As with earlier releases, restoring an old snapshot restores its old revocation state: review/revoke restored grants before reconnecting clients. No raw client tokens are stored in the database.
