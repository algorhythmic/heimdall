...
- Database marker 6, consistent pre-upgrade backups, exclusive live database backups, and fresh-directory recovery. A compiled 0.5.0 database upgraded with task/contract/checkpoint and read-grant retention and rolled back from its pre-upgrade snapshot successfully. Its existing read credential still refused checkpoint writes after upgrade.
...
| Event migration | Event envelope remains version 1. Database markers 1–5 upgrade to 6 after a consistent snapshot. Contracts support v1/v2; grants and checkpoints support old read/CLI v1 and scoped-write/client v2. Evaluator/evidence payloads begin at v1. Old binaries refuse marker 6. |
