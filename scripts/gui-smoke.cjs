...
(async()=>{
 const {root,exe,dir}=require('./smoke-paths.cjs').smokePaths('gui');
 const data=join(dir,'data');let daemon,browser;
 ...
 try{
  ...
  if(process.platform==='linux') {
   const artifact=cli('artifact','record','workspace','--expected-task-revision',revision,'--file',input('artifact-version',{artifact_id:'new',previous:'none',name:'Review artifact',environment:'gui-test',resource_id:resource.id,path:'.',git:false}));
   pins.artifacts=[{artifact_id:artifact.artifact.id,version_id:artifact.record.id}];
  }
  cli('checkpoint','create','workspace','--expected-task-revision',revision,'--file',input('checkpoint',{previous:'none',contract_id:contract.id,summary:'Evidence evaluation, replay and stale-input checks are implemented. The local task interface is ready for review.',next_action:'Inspect the task context and accept the verified completion proposal.',blockers:[],...pins}));
  ...
  if(pins.artifacts) {
   const detail=await page.evaluate(async()=> (await fetch('/ui/task?target=workspace')).json());
   assert.equal(detail.context.checkpoint.version,3);
   assert.deepEqual(detail.context.checkpoint.artifacts,pins.artifacts);
   assert.equal(detail.context.artifacts,undefined);
   assert(detail.context.issues.some(issue=>issue.code==='artifact_checks_require_cli'));
   await page.getByText('Artifact references are retained; live artifact/Git checks require the CLI',{exact:true}).waitFor();
  }
