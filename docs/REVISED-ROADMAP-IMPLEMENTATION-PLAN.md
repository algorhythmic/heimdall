...
This follows the [workflow assessment](OMARCHY-WORKFLOW-ASSESSMENT.md) and makes the recovery proposal concrete. Further GUI expansion, ranked planning, conversation ingestion, mail, and Braid do not block these increments. Keep the existing GUI working and reuse its completion review.

## 2. Starting point and evidence limits

| Area | Current source and recorded capability | Work to carry forward |
|---|---|---|
| Storage | Single writer, WAL/FULL, schema 9, immutable events, command receipts, one JSON state projection; consistent database backup | Extend the existing writer and migrations; measure snapshot volume before high-frequency capture. See [store](../internal/store/store.go). |
...
| Authority | Separate CLI/browser credentials; scoped reads and explicitly granted checkpoint writes; scoped GUI completion review | New adapter/result and workspace capabilities need explicit policy. Existing grants must not acquire them during migration. See [ledger](CAPABILITY-LEDGER.md). |
...
| Planning and external systems | Task/step hierarchy and local context work; no structured cross-project task dependency service, dotprivate adapter, WCU adapter, Braid integration, or execution host | Add these incrementally; repository presence or tool help is not integration acceptance. |
...
| `web/`, `internal/webui/` | Maintain existing views; expose recovery/progress details later from the same domain state. Run controls stay with C19. |
