...
## Current authority and remaining scope

Read credentials remain read-only. Checkpoint-write grants, MCP, evidence,
artifact identity and CLI/TUI decision review are delivered. Braid remains S3;
agent execution-host coordination is retired scope. Host installation is a
separate deployment action; dated schema-5 migration notes above remain historical.
