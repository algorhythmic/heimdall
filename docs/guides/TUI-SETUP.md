...
| g | On demand, read current browser attention through the authenticated active-state query. |
...
The selected context includes **Observed** browser rows only when an existing,
exact recorded tab owner still proves a reviewed browser-open action for that
task. The global attention line shows the latest recorded browser focus span for
each paired profile. `g` is an explicit one-shot read of `/state?active=1`; it
shows the active verdict, tab-level locator, selected task when exact ownership
permits it, coverage gaps and read age. It never polls, changes bindings or
credits focus to a task. Old epochs and unpaired profiles are historical; an
`invalid_pointer` observation is displayed as a gap. Compositor spans can join
this line when their S1 projection is available.
...
Current database schema is 22. Historical UI-v2 reviews still replay with their
