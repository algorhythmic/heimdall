...
Database markers 1–8 upgrade to 9. While holding the sole-writer lock, startup first publishes `backups/pre-schema-9-<id>.db`; a failed snapshot aborts the upgrade. Event envelopes remain version 1; new contracts are version 2, and legacy CLI checkpoints remain v1; artifact checkpoint requests use v2 and produce CLI records v3. Client checkpoints and write grants are v2 with authenticated grant provenance. Marker 6 introduced evaluator/evidence records; markers 7/8 add workspace declarations and observed Herdr bindings; marker 9 adds immutable artifact identities/versions and checkpoint pins. See [workspace setup](WORKSPACE-SETUP.md) and [Herdr setup](HERDR-SETUP.md). Old contracts replay unchanged but need explicit scope review before new checkpoints. Replay never reads resources or executes actions. Executables predating schema 9 refuse a marker-9 database.
