...
| P03 | Manual handoff and observation slice delivered | Checkpoint-pinned preview/request, independent source/mirror/commit/remote readback, distinct reported failures, portable export, schema-12 replay/retry/restore and scope guards. Programmatic dispatch and selected-clone serialization remain gated by C12 and a narrow dotprivate interface. See [preservation setup](PRESERVATION-SETUP.md). |
| P04 | Task dependencies and scoped summaries delivered | Immutable relation chains, both endpoint revisions, dependency/hierarchy cycle checks including atomic reparenting, explicit summary ordering/pagination, restricted foreign prerequisites, revocation and TUI status display. Planning relations do not add completion checks or dispatch agents. See [dependency setup](DEPENDENCY-SETUP.md). |
| W02 | Read-only Linux slice delivered | Hyprland 0.56.2 explicit source/viewport binding, native stable IDs and peer/boot/socket epochs, buffered bootstrap and periodic reconciliation, named workspace/monitor mapping, gap/stale diagnostics, scope isolation, inert retry/replay and restart/backup. Browser pairing awaits C13; durable snapshots and preview follow in W03/W04. See [Hyprland setup](HYPRLAND-SETUP.md). |
...
P02 CLI/TUI progress review and Neovim inspection/handoff are implemented on the terminal/editor and P01 artifact foundation. P03 manual preservation is delivered. P04 task dependencies and scoped summaries are delivered. Next is R3/W02 workspace observation, alongside user workflow baseline measurements and later review interface expansion. C03/evidence gaps are completed where the new slices consume them; C12 becomes shared infrastructure for verified recovery and WCU. P01 checkpoint pins include optional local Linux artifact/Git identity; programmatic preservation and broader remote/platform support remain open. Editor usage is in [NEOVIM-SETUP.md](NEOVIM-SETUP.md); other interfaces are documented in [GUI-SETUP.md](GUI-SETUP.md), [EVIDENCE-SETUP.md](EVIDENCE-SETUP.md), [MCP-SETUP.md](MCP-SETUP.md), [SCOPED-ACCESS.md](SCOPED-ACCESS.md) and [CONTINUITY-SETUP.md](CONTINUITY-SETUP.md).
