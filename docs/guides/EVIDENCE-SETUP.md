...
The daemon can observe registered artifacts, inspect repository state, and execute an explicitly configured test. Results are evidence for a task or step, not automatic completion. Review proposals with the `ratify` command or the [TUI](TUI-SETUP.md); both paths revalidate evidence before accepting completion.
...
- Configuration/evaluation routes accept only the unrestricted local CLI credential. Browser and scoped MCP credentials cannot configure commands, execute tests, or submit an invented passing outcome. This does not grant MCP test-execution authority; see the current [MCP tools](MCP-SETUP.md).
...
- The [TUI](TUI-SETUP.md) supports evidence inspection, explicit evaluation and completion review through local authority. Configuration is documented through the CLI and YAML workflows. Raw-output retention, broader machine evidence tools, automatic continuous invalidation and richer post-completion review notices remain later work. Unknown/unavailable evidence cannot establish completion.
- Historically, database schema marker 6 created a consistent `backups/pre-schema-6-*.db` snapshot before upgrading markers 1–5. Restore into a fresh directory for rollback; old binaries refuse marker 6. Backups retain user data and credential verifiers, so review grants after restoration.
