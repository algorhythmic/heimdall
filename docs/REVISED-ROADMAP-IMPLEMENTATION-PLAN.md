...
|---|---|---|
| Storage | Single writer, WAL/FULL, schema 11, immutable events, command receipts, one JSON state projection; consistent database backup | Extend the existing writer and migrations; measure snapshot volume before high-frequency capture. See [store](../internal/store/store.go). |
