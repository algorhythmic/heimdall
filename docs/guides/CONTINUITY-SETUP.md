...
The CLI stores accepted objectives, constraints, decisions, resource bindings and immutable checkpoints separately from `tasks.yaml`. Checkpoint traffic does not change the task document revision. The September 8 R0/T01 development slice adds readable resume and editable checkpoint drafts. Scoped MCP, evidence evaluators and the TUI are available. Braid is planned for
S3; agent execution is retired scope, not pending implementation.
