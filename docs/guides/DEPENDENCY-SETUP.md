...
Dependencies were introduced in schema 13. Current schema 21 preserves previous event families and exact receipts; startup publishes
`backups/pre-schema-21-*.db` before upgrading older data. Rollback uses the stopped
pre-upgrade snapshot and its matching old binary. See [request schema](../../schemas/dependency-request-v1.schema.json),
[verification](../VERIFICATION.md) and [roadmap](../HEIMDALL-IMPLEMENTATION-ROADMAP.md).
