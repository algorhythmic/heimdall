...
func (s *Store) TransactChecked(ctx context.Context, id, actor string, request []byte, now time.Time, authorize func(model.State) error, build func(model.State) (Change, error)) (json.RawMessage, error) {
	...
	if (actor == "scheduler" || actor == "coordinator" || actor == "observer:browser" || actor == "observer:herdr" || actor == sourceActor) && len(change.Events) == 0 {
		return result, nil
	}
...
func Apply(st *model.State, e Event) error {
	...
	case "workspace.residency_observed":
		if err := applyWorkspaceResidency(st, e); err != nil {
			return err
		}
	case "workspace.operation_queued", "workspace.diffed", "workspace.operation_cancelled", "workspace.operation_reconciled", "workspace.operation_settled", "workspace.closed":
		if err := applyWorkspaceOperation(st, e); err != nil {
			return err
		}
	...
	case "evaluator.accepted", "evidence.started", "evidence.finished", "evidence.invalidated":
		if err := applyEvidence(st, e); err != nil {
			return err
		}
