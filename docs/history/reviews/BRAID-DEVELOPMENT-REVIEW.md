...
Braid is **not integrated into Heimdall's runtime**. Heimdall has no retrieval adapter package or Braid import/process supervisor in its current `cmd/` or `internal/` code. Existing integration work consists of the [adapter contract](../design-v1/BRAID-CONTRACT.md), a standalone subprocess verification script in Braid, and planned items C14–C16 in the [continuity backlog](../../BACKLOG.md).
...
In query.go (historical source path: `../../../../Braid Retrieval Engine/pkg/braid/query.go`; unavailable checkout), MMR computes `lambda * fused_score - (1-lambda) * similarity`. Fused scores use reciprocal ranks; similarity uses cosine or **1 for any pair sharing a type when vectors are unavailable**. Shared type is not proof of redundant content. Small relevance values can be overwhelmed by that penalty. The repository's implementation notes already acknowledge the scale issue.
...
main.go (historical source path: `../../../../Braid Retrieval Engine/cmd/braid/main.go`; unavailable checkout) decodes requests with `json.Unmarshal`, accepts unknown fields, returns string errors and has no version/capability handshake. A misspelled or unsupported filter can therefore be silently ignored. This is particularly hazardous if a consumer mistakes an attribute boost or unsupported field for a scope restriction.
...
Filters (historical source path: `../../../../Braid Retrieval Engine/pkg/braid/types.go`; unavailable checkout) currently provide types, since and excluded IDs. These are not project/account authorization. Graph anchor traversal can use nodes omitted from final results, so merely hiding output IDs cannot isolate a project or account.
...
There is no public node deletion or edge-removal API. Existing replay (historical source path: `../../../../Braid Retrieval Engine/pkg/braid/projection.go`; unavailable checkout) removes unlogged direct upserts, by design. A consumer must not call it as a generic repair operation on a snapshot-fed index.
...
eval.go (historical source path: `../../../../Braid Retrieval Engine/pkg/braid/eval.go`; unavailable checkout) requires nonempty expected IDs. Its current recall/MRR/nDCG harness cannot directly represent a query for which no result is relevant. That matters for workstream assignment, stale decisions, unknown subjects and permission-limited corpora.
