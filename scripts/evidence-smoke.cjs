...
(async()=>{
 ...
 try{
  ...
  const started=run();const result=await finished('proof',started.id);assert.equal(result.outcome,'matched',JSON.stringify(result));assert.equal(result.exit_code,0);assert.equal(result.output_digest.length,64);assert.equal(result.executable_digest.length,64);
