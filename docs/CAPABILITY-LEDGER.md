...
C13 schema 18 adds explicit CLI action v2 pairing authority pinned to a selected compositor source and previous viewport head. Browser-ingress nonce metadata cannot directly bind a viewport: a coordinator joins two ordered browser/native observations, then commits the association and binding together. Continuations use separate durable delivery identities and pre-input extension journals. Scoped clients acquire no new capabilities. See [browser pairing](BROWSER-PAIRING.md).

C13 adds CLI-only action reconciliation and browser-ingress challenged readback. A daemon-owned matcher derives postconditions from exact issued instance references and a complete stable census; no caller can assert matched status. Monotonic leases expire on restart/connection change. Verification never accepts a task or grants resource/native-desktop authority. Ordinary browser inventory and old successes remain unverified; browser/compositor pairing is covered by the subsequent schema-18 handshake. Schema 17 preserves earlier credentials and receipts. See [browser verification](BROWSER-VERIFICATION.md).
