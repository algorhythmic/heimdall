...
(async()=>{
 const {root,exe,dir}=require('./smoke-paths.cjs').smokePaths('evidence');
 const data=join(dir,'data'),work=join(dir,'work');fs.mkdirSync(work);fs.writeFileSync(join(work,'input.txt'),'accepted input');let daemon;
 ...
 async function finished(target,evidence,timeoutSeconds){
  // The daemon allows the command timeout plus 10s of observation, then 5s
  // to persist its result. Wait for that full lifecycle, with 5s scheduling
  // margin, while still requiring the actual finished/matched record below.
  const waitMs=(timeoutSeconds+10+5+5)*1000,deadline=Date.now()+waitMs;
  let result;
  do{
   result=cli('evidence','list',target).evidence.find(e=>e.id===evidence);
   if(result?.status==='finished')return result;
   await wait(100);
  }while(Date.now()<deadline);
  throw Error(`evaluation ${evidence} did not finish within ${waitMs}ms; last record: ${JSON.stringify(result)}`);
 }
 try{
  ...
  const started=run();const result=await finished('proof',started.id,spec.timeout_seconds);assert.equal(result.outcome,'matched',JSON.stringify(result));assert.equal(result.exit_code,0);assert.equal(result.output_digest.length,64);assert.equal(result.executable_digest.length,64);
  ...
  const again=cli('evidence','evaluate','proof','--evaluator',definition.id,'--expected-task-revision',rev,'--request-id',id());const rerun=await finished('proof',again.id,spec.timeout_seconds);assert.equal(rerun.outcome,'matched',JSON.stringify(rerun));cli('tick');state=cli('state');const current=Object.values(state.proposals).find(p=>p.target==='proof'&&p.status==='pending');assert(current);cli('ratify',current.id,'--accept');assert.equal(cli('state','proof').task.status,'done');
