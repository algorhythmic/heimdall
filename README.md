...
**Current development: terminal/editor continuity, Herdr bindings, artifact versions and CLI/TUI progress review, extending 0.7.0 with database schema 13.** Resume an explicit task with its accepted direction, saved progress, blockers and file drift. Save progress through editable checkpoint drafts that preserve their original task, revision and retry identity.
...
The current binary upgrades database markers 1–12 to **13**, publishing a consistent `backups/pre-schema-13-*.db` before migration. Older binaries refuse marker 13. Database backups preserve recorded state and receipts; external working files require separate preservation. Follow [backup, upgrade and restore](docs/CONTINUITY-SETUP.md#backup-upgrade-and-restore) for fresh-directory recovery or rollback.
