...
restoration points. [W03 durable snapshots](SNAPSHOT-SETUP.md) now consume these observations; [W04 recovery previews](WORKSPACE-PREVIEW.md) and W05–W07 recovery are delivered.
...
Source selections and viewport bindings were introduced in schema 14; schema 15 added durable snapshots. Current schema 21 preserves
old grants, receipts and task completion authority. Upgrade first publishes a
consistent `backups/pre-schema-21-*.db`; older binaries refuse marker 21.
...
## Compositor attention fallback

The buffered socket2 observer records closed `surface.focused` and
`workspace.focused` intervals with qualified compositor payload fields. `state` exposes the latest span per source
epoch and per native window of the current epoch (at most 256 windows). Window
inventories are not written to the event log for attention. Intervals use Hyprland
event timing, have a two-second minimum and are dropped after a day. They are
not input timing, continuous focus proof, ownership, task binding or completion
evidence. Socket loss, compositor restart, observer restart and reconnection
discard an open interval rather than inventing blur.

`heimdall state --active --json` prefers fresh paired browser-tab focus. When
that coverage is unavailable it may use one fresh compositor `activewindow`
read and selects a task only from exact current recorded window ownership under
the reviewed manifest and task revision. Paired connected browser windows are
excluded. Class/workspace gaps are explicit; no prompt, transcript or input
timing is collected.
