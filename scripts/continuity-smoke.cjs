...
(async () => {
  ...
  try {
    ...
    if (legacy) assert.equal(fs.readdirSync(join(data, 'backups')).filter(n => n.startsWith('pre-schema-11-')).length, 1);
    ...
    if (legacy) {
      ...
      assert.notEqual(refused.status, 0); assert.match(refused.stderr, /unsupported database schema version 11/);
    }
    const source = data;
    ...
    if (legacy) {
      ...
      const pre = fs.readdirSync(join(source, 'backups')).find(n => n.startsWith('pre-schema-11-'));
